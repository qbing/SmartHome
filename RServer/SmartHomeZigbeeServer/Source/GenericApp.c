...
int my_print_str(char* str)
{
  return HalUARTWrite(HAL_UART_PORT_0,(unsigned char*)str,my_strlen(str));
}
...
UINT16 GenericApp_ProcessEvent( byte task_id, UINT16 events )
{
  ...
  /*********JUST FOR TEST**********begin*************/{
  static char flag = 1;
  if(GENERICAPP_TIMER_EVT==events)
  {
    ...
    while ( MSGpkt )
    {
      switch ( MSGpkt->hdr.event )
      {
        ...
        case SPI_INCOMING_ZAPP_DATA:
        {
          ...
          osal_event_hdr_t* msg_ptr = (osal_event_hdr_t*)MSGpkt;
          //shp_example_data_arrived_called_by_system((U8*)(msg_ptr+1),msg_ptr->status);
          sendData((U8*)(msg_ptr+1),msg_ptr->status);
...
#else
  /*****for SHP *************begin******************/{
   sendData((char*)pkt->cmd.Data,pkt->cmd.DataLength);
  /*****for SHP *************end********************/}
#endif
}
